...
void main(void)
{
	...
	CardReadInit();
	SysIdleCon();
	MP3_StartPlay(MP3_Systemactive);  //¿ª»úÆô¶¯
	//WdtConfig();
		...
		if(Timer0_Irq)
		{
			...
			if(++MainTime > 1000)
			{
				...
				if(SysStatus == SYS_TALK)
				{
					...
					//printf("TalkTime: %d\r\n",TalkTime);
					if(TalkTime && (TalkTime == SysPara.TalkTime))
					{
				...
				MainShow++;
				MainShow %= 15;	
				if((SysStatus == SYS_IDLE) && (IdleTime == 0) && (NameListIdx == 0xFFFF) && ((MainShow == 0) || (MainShow == 5) ||(MainShow == 10)))
				{
					ShowMain((MainShow==10)?1:((MainShow==5)?0:2));	
				}
			#endif
