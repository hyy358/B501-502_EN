...
void SysSettingCon(void)
{
	...
	DisplayString(2,1,StrTabSysSet1);
	DisplayIco(2,6,3);
	InputRoom = 0;
	InputIdx = 0;
}
...
void ShowSubPara(void)
{
	...
	if(SysStatus == SYS_SET_TONE)
	{	
		//sprintf(Disbuf,"%s%s",Disbuf,((InputRoom==0)?StrTabOff:StrTabOn));
		DisplayString(1,0,Disbuf);
		DisplayString(3,6,((InputRoom==1)?StrTabOn:StrTabOff));
		return;
	}
	else if((SysStatus == SYS_SET_CAMERA) || (SysStatus == SYS_SET_RINGIN))
	{
		DisplayString(1,0,Disbuf);
		DisplayString(2,6,((InputRoom==1)?StrTabOn:StrTabOff));
		return;
	}
//	else if(InputRoom > 0)
//	{
