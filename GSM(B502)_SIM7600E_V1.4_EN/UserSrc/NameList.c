	...
	if(tel)
	{
		if(pItem->Tel[i] > 0)
		{
			for(i=0;i<3;i++)
			{
				...
				#else
					SetFlash(1+i,(5 + strlen(pItem->Tel[i])));
				if((5 + strlen(pItem->Tel[i])) >= 15)
				{
					SetFlash(1+i,15);
				}
				#endif
...
void SaveSmsTel(void)
{
	...
	writeBuf_at24c02(SYS_PARA_ADDR+((uint16_t)((uint32_t)(&SysPara.SmsTel)-(uint32_t)(&SysPara))), SysPara.SmsTel,sizeof(SysPara.SmsTel));
	ShowSaveOK();
}
